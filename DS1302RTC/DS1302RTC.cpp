...
bool DS1302RTC::IsTimeElapsed(unsigned long seconds,bool onlyForOneTime)
{
	if (_timeToWait == 0)
	{
		_timeToWait = get() + seconds;
	}

	if (get() >= _timeToWait && !_isTimeElapsedBlocked)
	{
		if (onlyForOneTime)
		{
			_isTimeElapsedBlocked = true;
			return true;
		}
		else
		{
...
void DS1302RTC::ResetTimeElapsed()
{
	_timeToWait = 0;
	_isTimeElapsedBlocked = false;
}
