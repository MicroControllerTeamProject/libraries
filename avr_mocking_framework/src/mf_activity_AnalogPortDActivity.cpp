...
AnalogPortDActivity::AnalogPortDActivity(AvrMicroRepository& avrMicroRepository, AnalogPortD** analogPortForDigitalMeasure, uint8_t analogPortsNumber)
	: AnalogPortBActivity(avrMicroRepository, analogPortsNumber, (AnalogPortB**)analogPortForDigitalMeasure) {
...
bool AnalogPortDActivity::is_any_port_out_of_range() {
	...
	for (int i = 0; i < this->get_analog_ports_number(); i++) {
		if (this->analogPortForDigitalMeasure[i]->digital_value > this->analogPortForDigitalMeasure[i]->getMaxDigitalAlarmValueIn()) {
			...
			this->analogPortForDigitalMeasure[i]->is_time_above_threshold_elapsed =
				is_delay_elapsed_for_ports_above_threshold(this->analogPortForDigitalMeasure[i]);
			isOnAlarm = true;
		}
			...
			this->analogPortForDigitalMeasure[i]->is_onAlarm = true;
			this->analogPortForDigitalMeasure[i]->is_time_under_threshold_elapsed =
				is_delay_elapsed_for_ports_under_threshold(this->analogPortForDigitalMeasure[i]);
			isOnAlarm = true;
		}
		else {
