// File: mf_activity_AnalogPortBActivity.h
#pragma once
...
public:
	...
	uint16_t analog_read_average(uint8_t pin, uint8_t index_port, uint16_t number_of_read);
	uint16_t analog_read_raw_average(uint8_t pin, uint16_t number_of_read);

	void set_offset(uint8_t pin, uint8_t index_port, uint16_t number_of_read);
